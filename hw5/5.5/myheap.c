...
static void *coalesce(struct myheap *h, void *first_block_start) {
  ...
  if (!block_is_in_use(first_block_start) && (!block_is_in_use(next_block))) {
...
static int get_size_to_allocate(int user_size) {
  int payload_size = user_size % HEADER_SIZE? (user_size / HEADER_SIZE + 1) * HEADER_SIZE : user_size;
  return payload_size + HEADER_SIZE*2;
}
...
static void *split_and_mark_used(struct myheap *h, void *block_start, int needed_size) {
  ...
  if (block_size - needed_size >= HEADER_SIZE*3) {
	  ...
	  set_block_header(block_start+needed_size, block_size - needed_size, 0);
  } else {
...
void *myheap_malloc(struct myheap *h, unsigned int user_size) {
  void *current_block = h->start;
  int needed_size = get_size_to_allocate(user_size);
  do {
	  if (needed_size <= get_block_size(current_block)
		  && !block_is_in_use(current_block)
		  && current_block+needed_size <= h->start + h->size) {
		  void *temp = split_and_mark_used(h, current_block, needed_size);
		  return temp;
	  }
	  current_block = get_next_block(current_block);
  } while (is_within_heap_range(h, current_block));
  return NULL;
}