...
//    String hexValue = "";
//    hexValue = hexValue + String.format("%02X", byteAtAddrPlus0);
//    hexValue = hexValue + String.format("%02X", byteAtAddrPlus1);
//    hexValue = hexValue + String.format("%02X", byteAtAddrPlus2);
//    hexValue = hexValue + String.format("%02X", byteAtAddrPlus3);
//    return (int) Long.parseLong(hexValue, 16);

    return (byteAtAddrPlus0 << 8 * 3) & (0xff << 8 * 3)
            | (byteAtAddrPlus1 << 8 * 2) & (0xff << 8 * 2)
            | (byteAtAddrPlus2 << 8) & (0xff << 8)
...
//    hexString = ("00000000" + hexString).substring(hexString.length());
//    System.out.println(hexString);
//    for (int j = 0; j < bytes.length; j++) {
//      String substring = hexString.substring(j * 2, j * 2 + 2);
//      bytes[j] = Integer.valueOf(substring, 16).byteValue();
//    }
//    return bytes;
    byte[] bytes = new byte[4];
    for (int j = 0; j < bytes.length; j++) {
      bytes[j] = (byte) (i >> 8*(bytes.length-j-1));
    }
    for (int j = 0; j < bytes.length; j++) {
