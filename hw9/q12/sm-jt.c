...
void fetch() {
  ...
  static const void* jt[12] = { &&L0x0, &&CONT, &&CONT, &&CONT, &&CONT, &&CONT,
                                &&CONT, &&CONT, &&CONT, &&CONT, &&CONT, &&L0xb };
  if (insOpCode < 0x0 || insOpCode > 0xb) goto CONT;
  goto *jt[insOpCode];
L0x0:
L0xb:
...
int exec() {
  int cont = 1;
  int addr, val;
  static const void* jt[] = { &&L0x0, &&L0x1, &&L0x2, &&L0x3, &&L0x4, &&DEFAULT, &&L0x6, &&L0x7,
                              &&L0x8, &&L0x9, &&L0xa, &&L0xb, &&L0xc, &&DEFAULT, &&DEFAULT, &&L0xf };
  if (insOpCode < 0x0 || insOpCode > 0xf) goto DEFAULT;
  goto *jt[insOpCode];
L0x0: // ld $i, d .............. 0d-- iiii iiii
  reg [insOp0] = insOpExt;
  goto CONT;
L0x1: // ld o(rs), rd .......... 1osd
  addr = (insOp0 << 2) + reg [insOp1];
  reg [insOp2] = mem [addr] << 24 | mem [addr+1] << 16 | mem [addr+2] << 8 | mem [addr+3];
  goto CONT;
L0x2: // ld (rs, ri, 2), rd .... 2sid
  addr = reg [insOp0] + (reg [insOp1] << 2);
  reg [insOp2] = mem [addr] << 24 | mem [addr+1] << 16 | mem [addr+2] << 8 | mem [addr+3];
  goto CONT;
L0x3: // st rs, o(rd) .......... 3sod
...
L0x4: // st rs, (rd, ri, 4) .... 4sdi
  ...
  goto CONT;
L0x6: ; // ALU ................... 6-sd
  static const void* ijt[] = { &&IL0x0, &&IL0x1, &&IL0x2, &&IL0x3, &&IL0x4, &&IL0x5, &&IL0x6, &&IL0x7,
                              &&IDEFAULT, &&IDEFAULT, &&IDEFAULT, &&IDEFAULT, &&IDEFAULT, &&IDEFAULT, &&IDEFAULT, &&IL0xf };
  if (insOp0 < 0 || insOp0 > 0xf) goto IDEFAULT;
  goto *ijt[insOp0];
IL0x0: // mov rs, rd ........ 60sd
  reg [insOp2] = reg [insOp1];
  goto ICONT;
IL0x1: // add rs, rd ........ 61sd
  reg [insOp2] = reg [insOp1] + reg [insOp2];
  goto ICONT;
IL0x2: // and rs, rd ........ 62sd
  reg [insOp2] = reg [insOp1] & reg [insOp2];
  goto ICONT;
IL0x3: // inc rr ............ 63-r
